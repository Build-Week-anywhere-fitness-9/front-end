...
const ClassList = ({ user, classes, history, isFetching, error }) => {
  ...
  classes.forEach((oneClass) => {
    if (user === oneClass.owner) {
      focusClasses.push(oneClass);
    } else if (oneClass.participants.includes(user)) {
      focusClasses.push(oneClass);
    } else {
      otherClasses.push(oneClass);
    }
  });
        ...
        <div className="focusClass" key={idx}>
          ...
          {user === i.owner ? (
            ...
            <div className="ownerOptions">
              ...
              <button
                onClick={history.push.bind("/editClass")}
                className="editClass"
              >
                Edit Class
              </button>
